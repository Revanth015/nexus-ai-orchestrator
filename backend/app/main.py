...
from .ai_connections import list_connections, register_connection, update_connection, delete_connection, test_connection as test_custom_connection, diagnose_connection
...
async def upload_file(file: UploadFile = File(...)):
    try:
        saved = save_upload(file.filename or "uploaded_file", await file.read())
        saved.pop("stored_path", None)
        return {"status":"uploaded","file":saved}
...
def add_worker_connection(request: AIConnectionRequest):
    try: return {"status":"registered","worker":register_connection(**request.model_dump())}
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
@app.patch("/workers/connections/{worker_id}")
def edit_worker_connection(worker_id: str, request: AIConnectionUpdateRequest):
    try: return {"status":"updated","worker":update_connection(worker_id, **request.model_dump(exclude_unset=True))}
...
def test_worker_connection(worker_id: str, request: GeminiTestRequest | None = None):
    try: return test_custom_connection(worker_id, prompt=request.prompt if request else "Reply with exactly: NEXUS connection is working.")
...
def workers_learning():
    from .worker_learning import learning_snapshot
...
def route(task_type: str): return route_task(task_type, free_only=settings.free_only)
@app.post("/manager/decide/{task_type}", response_model=ManagerExecutionDecision)
def manager_decide_endpoint(task_type: str, request: PromptRequest): return decide_worker_for_task(task_type, prompt=request.prompt, free_only=settings.free_only)
@app.post("/missions")
...
def collaboration_plan(request: PromptRequest):
...
def execute_mission_endpoint(request: MissionExecutionRequest):
...
def gemini_connector_status(): return gemini_status()
