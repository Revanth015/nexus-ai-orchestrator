    ...
    try:
        ...
        for task in result.tasks:
            ...
            record_event(
                ...
                actor=task.worker_id or "nexus-manager",
                data={
                    "status": task.status,
                    "task_type": task.task_type,
                    "worker_id": task.worker_id,
                    "collaborators": task.collaborators,
                    "candidate_worker_ids": task.candidate_worker_ids,
                    "manager_confidence": task.manager_confidence,
                    "manager_estimated_value": task.manager_estimated_value,
                    "manager_resource_cost": task.manager_resource_cost,
                    "sprint": task.sprint,
            ...
            if task.manager_decision:
                decision = {
                    "task_id": task.task_id,
                    "decision": task.manager_decision,
                    "worker_id": task.worker_id,
                    "candidates": task.candidate_worker_ids,
                    "collaborators": task.collaborators,
                    "rationale": task.manager_rationale,
                    "confidence": task.manager_confidence,
                    "estimated_value": task.manager_estimated_value,
                ...
                decision_record(
                    ...
                    task_id=task.task_id,
                    task_type=task.task_type,
                    selected_worker_id=task.worker_id,
                    candidates=task.candidate_worker_ids,
                    decision=task.manager_decision,
                    rationale=task.manager_rationale or "Mission execution decision",
                    confidence=task.manager_confidence or 0,
                    expected_value=task.manager_estimated_value or 0,
                    resource_cost=task.manager_resource_cost or 0,
                )
            if task.quality_decision:
                finding = {
                    "task_id": task.task_id,
                    "recommendation": task.quality_decision,
                    "problem": task.rework_problem,
                    "quality_score": task.quality_score,
                    "rework_number": task.rework_number,
                }
                add_qa_finding(mission_id, finding)
                record_event(
                    "qa_rework" if task.quality_decision == "REWORK" else "qa_pass",
                    mission_id=mission_id,
                    task_id=task.task_id,
                    actor=task.worker_id or "reviewer",
                    data=finding,
                )
        ...
        update_mission(
            ...
            resource_used=sum(int(max(1, round(t.manager_resource_cost or 1))) for t in result.tasks if t.status in {"completed", "reviewed"}),
        )
        record_event(
        ...
        if result.status == "completed":
            ...
            record_event(
                "mission_ended",
                mission_id=mission_id,
                data={"status": result.status, "manager_decision": result.manager_decision, "rework_count": result.rework_count, "final_quality": final_quality},
            )
        return result
