...
def execute_mission_with_memory(request: MissionExecutionRequest, *, free_only: bool = True) -> MissionExecutionResponse:
    ...
    def sync_state(state: str, data: dict) -> None:
        ...
        update_mission(mission_id, current_task_id=data.get("task_id"), sprint=data.get("sprint", mission.get("sprint", 1)))
        record_event("mission_state", mission_id=mission_id, task_id=data.get("task_id"), data={"state": state, **data})
        if data.get("workers"):
            current = list(get_mission_workers())
            for worker_id in data["workers"]:
                if worker_id and worker_id not in current: current.append(worker_id)
            update_mission(mission_id, active_workers=current)

    def get_mission_workers() -> list[str]:
        from .mission_memory import get_mission
        current = get_mission(mission_id) or {}
        return list(current.get("active_workers", []))
    ...
    try:
        ...
        for task in result.tasks:
            ...
            if task.worker_id:
                current = get_mission_workers()
                for worker_id in [task.worker_id, *task.collaborators]:
                    if worker_id and worker_id not in current: current.append(worker_id)
                update_mission(mission_id, active_workers=current)
            record_event("task_execution", mission_id=mission_id, task_id=task.task_id, actor=task.worker_id or "nexus-manager", data={"status": task.status, "task_type": task.task_type, "worker_id": task.worker_id, "collaborators": task.collaborators, "sprint": task.sprint, "rework_number": task.rework_number, "error": task.error})
            if task.manager_decision:
                decision = {"task_id": task.task_id, "decision": task.manager_decision, "worker_id": task.worker_id, "collaborators": task.collaborators, "rationale": task.manager_rationale}
                add_decision(mission_id, decision)
                decision_record(mission_id=mission_id, task_id=task.task_id, task_type=task.task_type, selected_worker_id=task.worker_id, candidates=task.collaborators, decision=task.manager_decision, rationale=task.manager_rationale or "Mission execution decision", confidence=0, expected_value=task.route_score or 0, resource_cost=1)
            if task.quality_decision:
                finding = {"task_id": task.task_id, "recommendation": task.quality_decision, "problem": task.rework_problem, "quality_score": task.quality_score, "rework_number": task.rework_number}
            ...
            if task.status in {"completed", "reviewed"}: record_resource_use(mission_id, 1)
