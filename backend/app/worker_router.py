...
class WorkerRouteResponse(BaseModel):
    ...
    routing_policy: str = "dynamic_task_specific_performance_v7"
...
def _score(worker: WorkerProfile, capability: str, task_type: str) -> tuple[float, dict[str,float], bool]:
    ...
    # Low-confidence workers retain a small exploration advantage so a new worker can earn evidence.
...
def route_task(task_type: str, *, free_only: bool=True) -> WorkerRouteResponse:
    ...
    workers=[w for w in list_workers() if w.enabled]
