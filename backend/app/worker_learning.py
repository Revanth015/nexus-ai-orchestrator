...
def task_performance(worker_id,task_type):
    ...
    sr=t.get("successes",0)/n;rev=int(t.get("quality_passes",0))+int(t.get("quality_reworks",0));rr=int(t.get("quality_reworks",0))/rev if rev else 0;lat=t.get("latencies_ms",[]);avg=sum(lat)/len(lat) if lat else 0;qs=t.get("quality_scores",[]);quality=sum(qs)/len(qs) if qs else sr*100;last=t.get("last_observed_at");age=999 if not last else max(0,(datetime.now(timezone.utc)-datetime.fromisoformat(last.replace("Z","+00:00"))).total_seconds()/86400);rec=math.exp(-age/30) if age<999 else 0;score=max(0,min(100,quality*.55+sr*100*.25+(100-rr*100)*.15+max(0,min(100,100-avg/100))*.05));conf=min(100,((1-math.exp(-n/12))*.65+rec*.20+max(0,1-min(1,(t.get("failures",0)/n)*1.5))*.15)*100);return {"score":round(score,2),"confidence":round(conf,2),"success_rate":round(sr*100,2),"rework_rate":round(rr*100,2),"avg_latency_ms":round(avg,2),"observations":n,"quality_score":round(quality,2),"recency_factor":round(rec,4)}
...
def run_self_initialization(worker_ids=None,force=False):
    ...
    from .ai_connectors import generate_claude,generic_perplexity if False else generate_perplexity
    ...
    for w in list_workers():
        ...
        for cap in o.get("benchmark_capabilities",[]):
            for test in cap.get("tests",[]):
                ...
                except Exception as exc:tests.append({"test_id":test["test_id"],"capability":cap["capability"],"metric":test["metric"],"status":"failed","score":0.0,"error":str(exc)[:300]})
