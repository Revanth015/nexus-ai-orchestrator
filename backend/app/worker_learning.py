...
def _load() -> dict[str, Any]:
    try:
        ...
        data.setdefault("workers", {}); data.setdefault("collaboration", {})
        data["version"] = max(int(data.get("version", 1)), 5)
        return data
    except (FileNotFoundError, json.JSONDecodeError):
        return {"workers": {}, "version": 5, "collaboration": {}}
...
def record_result(worker_id: str, *, task_type: str, success: bool, latency_ms: float | None = None, quality: str | None = None, quality_score: float | None = None) -> None:
    with _LOCK:
        data = _load(); worker = data.setdefault("workers", {}).setdefault(worker_id, _default_worker())
        worker["observations"] += 1; worker["successes"] += int(success); worker["failures"] += int(not success)
        if latency_ms is not None: worker["latencies_ms"] = (worker.get("latencies_ms", []) + [round(float(latency_ms), 2)])[-50:]
...
def record_collaboration(source_worker_id: str, support_worker_id: str, *, task_type: str, success: bool, latency_ms: float | None = None) -> None:
    with _LOCK:
        data = _load(); key = f"{source_worker_id}::{support_worker_id}"
...
def task_performance(worker_id: str, task_type: str) -> dict[str, float]:
    worker = get_worker_learning(worker_id); task = worker.get("tasks", {}).get(task_type, {}); observations = int(task.get("observations", 0))
    if not observations: return {"score": 0.0, "confidence": 0.0, "success_rate": 0.0, "rework_rate": 0.0, "avg_latency_ms": 0.0, "observations": 0}
    success_rate = task.get("successes", 0) / observations; reviewed = int(task.get("quality_passes", 0)) + int(task.get("quality_reworks", 0))
...
def self_initialize() -> dict[str, Any]:
    """Create onboarding benchmark plans for newly discovered workers without resetting evidence."""
    ...
    with _LOCK:
        ...
        for worker in list_workers():
            ...
            if "onboarding" not in existing:
                ...
                applicable = [name for name, value in caps.items() if value is not None and name in _CAPABILITY_TESTS]
...
def _evaluate(test_id: str, text: str) -> tuple[float, list[str]]:
    """Score objective benchmark characteristics; this is not an LLM judging itself."""
    text = text.strip(); lower = text.lower(); checks: list[bool] = []
    if test_id == "reasoning_logic_01": checks = ["no" in lower and "a" in lower and "c" in lower]
    elif test_id == "reasoning_constraints_01":
        lines = [x.strip() for x in text.splitlines() if x.strip()]; checks = [len(lines) == 3, all(len(re.findall(r"\b\w+\b", x)) == 5 for x in lines)]
    elif test_id == "research_synthesis_01": checks = [all(k in lower for k in ["evidence", "source", "conflict"]), "do not invent" not in lower or "not invent" in lower]
    ...
    score = round(sum(checks) / len(checks) * 100, 2) if checks else 0.0
    return score, [f"check_{i+1}:{'pass' if ok else 'fail'}" for i, ok in enumerate(checks)]


def run_self_initialization(worker_ids: list[str] | None = None, force: bool = False) -> dict[str, Any]:
    """Execute onboarding benchmarks only for new workers; force is available for explicit retesting."""
    from .worker_registry import list_workers
    ...
    for worker in list_workers():
        ...
        profile = data["workers"].get(worker.worker_id, {}); onboarding = profile.get("onboarding", {})
        if onboarding.get("status") == "completed" and not force:
            results.append({"worker_id":worker.worker_id,"status":"skipped_existing_benchmark","reason":"existing worker history preserved"}); continue
        ...
        for capability in onboarding.get("benchmark_capabilities", []):
            for test in capability.get("tests", []):
                ...
                try:
                    if worker.worker_id == "gemini": response = generate_gemini(test["prompt"])
                    elif worker.worker_id == "claude": response = generate_claude(test["prompt"])
                    else: response = generate_perplexity(test["prompt"])
                    score, checks = _evaluate(test["test_id"], response.get("text", ""))
                    tests.append({"test_id":test["test_id"],"capability":capability["capability"],"metric":test["metric"],"status":"completed","score":score,"checks":checks,"latency_ms":response.get("telemetry",{}).get("last_latency_ms") or response.get("latency_ms")})
                except Exception as exc:
                    tests.append({"test_id":test["test_id"],"capability":capability["capability"],"metric":test["metric"],"status":"failed","score":0.0,"error":str(exc)[:300]})
        with _LOCK:
            data = _load(); profile = data["workers"].setdefault(worker.worker_id, _default_worker()); ob = profile.setdefault("onboarding", onboarding)
            ob["status"] = "completed" if tests and all(t["status"] == "completed" for t in tests) else "partial"
            ob["completed_at"] = now; ob["tests"] = tests; ob["tests_completed"] = sum(t["status"] == "completed" for t in tests)
            by_cap = {}
            for t in tests: by_cap.setdefault(t["capability"], []).append(t["score"])
            ob["benchmark_scores"] = {cap: round(sum(vals)/len(vals),2) for cap, vals in by_cap.items()}
            _save(data)
        results.append({"worker_id":worker.worker_id,"status":ob["status"],"tests_completed":ob["tests_completed"],"tests_total":ob.get("tests_total",len(tests)),"benchmark_scores":ob["benchmark_scores"],"tests":tests})
