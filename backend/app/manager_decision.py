...
def decide(*, task_type: str, complexity: float, confidence: float, quality_risk: float, worker_score: float, collaboration_score: float = 0.0, latency_ms: float = 0.0, budget_remaining: int = 10, evidence_gap: float = 0.0) -> ManagerDecision:
    """Choose the smallest workforce likely to achieve the mission objective."""
    ...
    collaboration = (complexity >= 65 and collaboration_score >= 60) or evidence_gap >= 55
    ...
    if worker_score < 60:
        ...
        rationale = "Task complexity, quality risk, or low confidence justifies independent verification."
    elif collaboration:
        action = "COLLABORATE"
        rationale = "Evidence gap or task complexity makes a second employee likely to add measurable value."
    else:
        action = "ASSIGN"
        rationale = "A proven worker can complete the task without unnecessary additional AI calls."
