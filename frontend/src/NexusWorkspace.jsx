...
function connectionState(worker) {
  if (!worker?.metadata?.custom) return "Built-in";
  ...
  return <main className="nexus-workspace">
    ...
    <div className="workspace-body">
      <aside className="workspace-sidebar"><button className="new-chat" onClick={() => setMessages([])}><MessageSquarePlus size={17} /> New chat</button><div className="sidebar-label">AI WORKFORCE</div>
        ...
        {workers.map(worker => { const ready = !!worker.execution_ready; const builtIn = !worker.metadata?.custom; return <button key={worker.worker_id} className={`model-item ${selectedWorker === worker.worker_id ? "active" : ""} ${!ready ? "disabled" : ""}`} onClick={() => selectWorker(worker)} title={!ready ? (builtIn ? "Built-in worker; configure its provider connector before selecting it" : "Connect and successfully test this AI before selecting it") : "Select this AI"}><span className={`model-dot ${ready ? "online" : ""}`} /><span><strong>{worker.name}</strong><small>{worker.model ?? worker.metadata?.model ?? "Model"}{builtIn ? " · Built-in" : !ready ? " · Not connected" : " · Connected"}</small></span></button>; })}
