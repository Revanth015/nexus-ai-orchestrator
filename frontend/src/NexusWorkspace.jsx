...
import { BrainCircuit, ChevronDown, MessageSquarePlus, Paperclip, Pencil, Plus, Send, Settings, Trash2, X } from "lucide-react";
...
export default function NexusWorkspace() {
  ...
  const loadWorkers = async () => {
    try {
      ...
      setWorkers(list);
      setSelectedWorker(current => current && list.some(w => w.worker_id === current) ? current : list.find(w => w.execution_ready)?.worker_id ?? list[0]?.worker_id ?? "");
    } catch (e) {
      setError(e.message || "Could not load AIs");
    }
  };
  ...
  const saveConnection = async event => {
    ...
    try {
      const editingId = editing?.worker_id;
      const payload = { ...form };
      if (editingId && !payload.api_key) delete payload.api_key;
      const response = await fetch(`${API_BASE}/workers/connections${editingId ? `/${editingId}` : ""}`, {
        method: editingId ? "PATCH" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(typeof data.detail === "string" ? data.detail : "Could not save AI connection");
      setShowForm(false); setEditing(null); setForm(emptyForm); await loadWorkers();
    } catch (e) { setError(e.message || "Could not save AI connection"); }
  ...
  const diagnose = async worker => {
    ...
    try {
      const response = await fetch(`${API_BASE}/workers/connections/${worker.worker_id}/diagnose`, { method: "POST" });
      const data = await response.json();
      if (!response.ok) throw new Error(typeof data.detail === "string" ? data.detail : "Diagnosis failed");
      setDiagnoses(d => ({ ...d, [worker.worker_id]: data }));
      await loadWorkers();
    } catch (e) { setError(e.message || "Diagnosis failed"); }
    finally { setDiagnosing(s => ({ ...s, [worker.worker_id]: false })); }
  };

  const removeConnection = async worker => {
    if (!window.confirm(`Remove ${worker.name}?`)) return;
    try {
      const response = await fetch(`${API_BASE}/workers/connections/${worker.worker_id}`, { method: "DELETE" });
      if (!response.ok) { const data = await response.json(); throw new Error(data.detail || "Could not remove AI"); }
      setDiagnoses(d => { const next = { ...d }; delete next[worker.worker_id]; return next; });
      await loadWorkers();
  ...
  const upload = async selected => {
    ...
    try {
      for (const file of Array.from(selected)) {
        ...
        const response = await fetch(`${API_BASE}/files/upload`, { method: "POST", body });
        const data = await response.json();
  ...
  const send = async event => {
    ...
    if (!worker) { setError("Connect and select an AI first."); return; }
    setRunning(true); setError(""); setPrompt("");
    setMessages(current => [...current, { role: "user", content: text, worker: worker.name, files: files.map(f => f.filename) }]);
    try {
      const response = await fetch(`${API_BASE}/execute`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ task_type: "general_reasoning", prompt: text, file_ids: files.map(f => f.file_id), forced_worker_id: selectedWorker, allow_fallback: true }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(typeof data.detail === "string" ? data.detail : "AI request failed");
      setMessages(current => [...current, { role: "assistant", content: data.output, worker: data.worker_name, fallback: data.fallback_used, attempts: data.attempts }]);
      setFiles([]);
      if (data.worker_id !== selectedWorker) setSelectedWorker(data.worker_id);
  ...
  return <main className="nexus-workspace">
    <header className="workspace-topbar">
      <div className="workspace-brand"><span className="workspace-logo"><BrainCircuit size={21} /></span><div><strong>NEXUS</strong><small>AI WORKSPACE</small></div></div>
      <div className="workspace-actions"><button onClick={() => setShowConnections(true)}><Settings size={16} /> AI connections <span>{workers.length}</span></button></div>
    </header>

    <div className="workspace-body">
      <aside className="workspace-sidebar">
        <button className="new-chat" onClick={() => setMessages([])}><MessageSquarePlus size={17} /> New chat</button>
        <div className="sidebar-label">CONNECTED AIs</div>
      ...
      <section className="workspace-chat">
        {messages.length === 0 ? <div className="welcome"><div className="welcome-icon"><BrainCircuit size={28} /></div><h1>Work with your AIs.</h1><p>Connect the models you use and choose the right one for each task. NEXUS keeps the workspace simple; the AIs do the work.</p>{workers.length === 0 && <button onClick={openAdd}><Plus size={16} /> Connect your first AI</button>}</div> : <div className="message-list">{messages.map((message, index) => <article className={`message ${message.role}`} key={index}><div className="message-meta">{message.role === "user" ? "You" : message.worker}{message.files?.length ? ` · ${message.files.join(", ")}` : ""}</div><div className="message-content">{message.content}</div>{message.role === "assistant" && message.fallback && <small className="fallback-note">NEXUS switched to {message.worker} after a failed attempt.</small>}</article>)}</div>}
        ...
        <form className="chat-composer" onSubmit={send}>
          ...
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder={selected ? `Ask ${selected.name} anything...` : "Connect an AI to start..."} rows={3} disabled={running} />
          <div className="composer-footer"><input ref={fileInputRef} type="file" hidden accept=".csv,.xlsx,.xlsm,.pdf,.txt" multiple onChange={e => upload(e.target.files)} /><button type="button" className="icon-button" onClick={() => fileInputRef.current?.click()} title="Attach file"><Paperclip size={17} /></button><div className="model-select"><ChevronDown size={14} /><select value={selectedWorker} onChange={e => setSelectedWorker(e.target.value)} disabled={!workers.length}>{workers.length === 0 ? <option value="">No AI connected</option> : workers.map(w => <option value={w.worker_id} key={w.worker_id}>{w.name} · {w.model ?? w.metadata?.model ?? "model"}</option>)}</select></div><button className="send-button" disabled={!prompt.trim() || running || !selectedWorker}>{running ? "Working..." : <><Send size={16} /> Send</>}</button></div>
        </form>
      </section>
    </div>
